...
@section('main_class', 'flex-1 max-w-7xl w-full mx-auto px-3.5 sm:px-6 lg:px-8 py-2 md:py-7 pb-20 md:pb-8 h-[calc(100dvh-5rem)] md:h-auto overflow-hidden md:overflow-visible')
...
@section('content')
    ...
    <!-- ========================================================================= -->
    <!-- 1. TAMPILAN MOBILE: 3 BAGIAN, SATU LAYAR TANPA SCROLL (md:hidden)        -->
    <!--    (Fokus pada Alert, Tombol Pinjam, dan Tombol Kembali)                 -->
    <!-- ========================================================================= -->
    <div class="md:hidden flex flex-col justify-between h-full min-h-0 gap-3">

        <!-- BAGIAN 1: ALERT MOBILE -->
        <div class="shrink-0">
            @if (session('success'))
                <!-- Flash Message Success -->
                <div class="bg-emerald-50 border-2 border-emerald-300 text-emerald-950 rounded-2xl p-3 flex items-center justify-between gap-3 shadow-2xs">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[11px] text-emerald-900 font-medium truncate">{{ session('success') }}</p>
                        </div>
                    </div>
                </div>
            @elseif (session('error'))
                <!-- Flash Message Error -->
                <div class="bg-rose-50 border-2 border-rose-300 text-rose-950 rounded-2xl p-3 flex items-center justify-between gap-3 shadow-2xs">
                    <div class="flex items-center gap-2.5 min-w-0">
                        <div class="w-8 h-8 rounded-xl bg-rose-600 text-white flex items-center justify-center shrink-0 shadow-2xs">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[11px] text-rose-900 font-medium truncate">{{ session('error') }}</p>
                        </div>
                    </div>
                </div>
            @elseif ($hasOverdue)
                <!-- Alert Peringatan Terlambat (High Priority) -->
                <div class="bg-rose-50 border-2 border-rose-300 text-rose-950 rounded-2xl p-3 flex items-center justify-between gap-2.5 shadow-2xs animate-pulse">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-1.5">
                                <span class="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-rose-200 text-rose-950 border border-rose-300">
                                    Terlambat
                                </span>
                            ...
                            <p class="text-[11px] text-rose-900 font-bold truncate mt-0.5">
                                Segera serahkan alat fisik ke meja Toolman bengkel.
                            </p>
                        </div>
                    </div>
                    <a href="{{ route('peminjam.tiket.index', ['status' => 'terlambat']) }}"
                        class="inline-flex items-center px-2.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-extrabold text-[11px] rounded-xl shrink-0 shadow-2xs transition-colors">
                        Lihat &rarr;
                    </a>
                </div>
            @elseif ($countActive > 0)
                <!-- Alert Status Peminjaman Sedang Aktif -->
                <div class="bg-blue-50 border-2 border-blue-200 text-blue-950 rounded-2xl p-3 flex items-center justify-between gap-2.5 shadow-2xs">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h4 class="font-black text-xs text-blue-950 truncate">
                                {{ $countActive }} Tiket Sedang Anda Pinjam
                            </h4>
                            <p class="text-[11px] text-blue-800 font-medium truncate mt-0.5">
                                Gunakan secara bertanggung jawab dan kembalikan tepat waktu.
                            </p>
                        </div>
                    </div>
                    <a href="{{ route('peminjam.tiket.index', ['status' => 'active']) }}"
                        class="inline-flex items-center px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-[11px] rounded-xl shrink-0 shadow-2xs transition-colors">
                        Tiket &rarr;
                    </a>
                </div>
            @elseif ($countPending > 0)
                <!-- Alert Pengajuan Menunggu Konfirmasi -->
                <div class="bg-amber-50 border-2 border-amber-200 text-amber-950 rounded-2xl p-3 flex items-center justify-between gap-2.5 shadow-2xs">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[11px] text-amber-800 font-medium truncate mt-0.5">
                                Permohonan peminjaman Anda sedang ditinjau di bengkel.
                            </p>
                        </div>
                    </div>
                    <a href="{{ route('peminjam.tiket.index', ['status' => 'pending']) }}"
                        class="inline-flex items-center px-2.5 py-1.5 bg-amber-600 hover:bg-amber-700 text-white font-extrabold text-[11px] rounded-xl shrink-0 shadow-2xs transition-colors">
                        Status &rarr;
                    </a>
                </div>
            @else
                <!-- Alert Status Normal / Bersih (Semua Aman) -->
                <div class="bg-emerald-50/80 border-2 border-emerald-200 text-emerald-950 rounded-2xl p-3 flex items-center justify-between gap-2.5 shadow-2xs">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h4 class="font-black text-xs text-emerald-950 truncate">
                                Status Peminjaman Aman &bull; {{ $bengkel->nama ?? 'SMKN 3 YK' }}
                            </h4>
                            <p class="text-[11px] text-emerald-800 font-medium truncate mt-0.5">
                                Halo, {{ strtok($user->name, ' ') }}! Tidak ada tanggungan alat bengkel saat ini.
                            </p>
                        </div>
                    </div>
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-900 border border-emerald-300 shrink-0">
        ...
        <!-- BAGIAN 2 & 3: DUA TOMBOL UTAMA MOBILE (PINJAM & KEMBALIKAN) -->
        <div class="grid grid-cols-1 gap-3 flex-1 min-h-0">

            <!-- BAGIAN 2: TOMBOL PINJAM MOBILE -->
            <a href="{{ route('peminjam.katalog.index') }}"
                class="group relative bg-white hover:bg-emerald-50/40 active:scale-[0.99] rounded-2xl p-3.5 border-2 border-emerald-300 hover:border-emerald-600 shadow-sm hover:shadow-md transition-all flex flex-col justify-between overflow-hidden">
                <div class="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-emerald-500 to-teal-400"></div>

                <div class="flex items-center gap-3.5">
                    <div class="w-16 h-16 rounded-xl overflow-hidden bg-emerald-50 p-1 border border-emerald-200 shrink-0 shadow-2xs group-hover:scale-105 transition-transform">
                        <img src="{{ asset('images/dashboard/pinjam.jpg') }}" alt="Pinjam Alat"
                            class="w-full h-full object-cover rounded-lg">
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-1 mb-1">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-900 border border-emerald-300">
                                Katalog Alat & Bahan
                            </span>
                        </div>
                        <h3 class="text-xl font-black text-gray-900 group-hover:text-emerald-700 transition-colors leading-tight">
                            Pinjam
                        </h3>
                        <p class="text-[11px] text-gray-600 font-medium line-clamp-2 mt-0.5 leading-relaxed">
                            Cari dan ajukan peminjaman perkakas tangan, mesin, alat ukur, dan bahan habis pakai praktik.
                        </p>
                    </div>
                </div>

                <div class="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between">
                    <span class="text-xs font-bold text-gray-500 group-hover:text-emerald-700 transition-colors">
                        Ajukan Pinjaman
                    </span>
                    <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-600 group-hover:bg-emerald-700 text-white font-extrabold text-xs shadow-xs transition-all">
            ...
            <!-- BAGIAN 3: TOMBOL KEMBALIKAN MOBILE -->
            <a href="{{ route('peminjam.tiket.index') }}"
                class="group relative bg-white hover:bg-blue-50/40 active:scale-[0.99] rounded-2xl p-3.5 border-2 border-blue-300 hover:border-blue-600 shadow-sm hover:shadow-md transition-all flex flex-col justify-between overflow-hidden">
                <div class="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-blue-500 to-indigo-500"></div>

                <div class="flex items-center gap-3.5">
                    <div class="w-16 h-16 rounded-xl overflow-hidden bg-blue-50 p-1 border border-blue-200 shrink-0 shadow-2xs group-hover:scale-105 transition-transform">
                        <img src="{{ asset('images/dashboard/kembali.jpg') }}" alt="Kembalikan Alat"
                            class="w-full h-full object-cover rounded-lg">
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-1 mb-1">
                            @if ($hasOverdue)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-100 text-rose-900 border border-rose-300 animate-pulse">
                                    <span class="w-1.5 h-1.5 rounded-full bg-rose-600 mr-1 animate-ping"></span>
                                    Perlu Dikembalikan!
                                </span>
                            @elseif ($countActive > 0)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-900 border border-blue-300">
                                    <span class="w-1.5 h-1.5 rounded-full bg-blue-600 mr-1 animate-pulse"></span>
                                    {{ $countActive }} Sedang Dipinjam
                                </span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-gray-100 text-gray-800 border border-gray-300">
                                    Tiket Peminjaman
                                </span>
                            @endif
                        </div>
                        <h3 class="text-xl font-black text-gray-900 group-hover:text-blue-700 transition-colors leading-tight">
                            Kembalikan
                        </h3>
                        <p class="text-[11px] text-gray-600 font-medium line-clamp-2 mt-0.5 leading-relaxed">
                            @if ($countActive > 0)
                                Pantau tenggat waktu, ajukan pengembalian, dan serahkan alat fisik ke Toolman bengkel.
                            @else
                                Lihat riwayat tiket peminjaman, status verifikasi Toolman, dan arsip sirkulasi bengkel.
                            @endif
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between">
                    <span class="text-xs font-bold {{ $hasOverdue ? 'text-rose-600 font-extrabold' : 'text-gray-500 group-hover:text-blue-700' }} transition-colors">
                        @if ($hasOverdue)
                            Ada Keterlambatan
                        @elseif ($countActive > 0)
                            {{ $countActive }} Tiket Aktif
                        @else
                            Semua Tuntas
                        @endif
                    </span>
                    <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl {{ $hasOverdue ? 'bg-rose-600 group-hover:bg-rose-700' : 'bg-blue-600 group-hover:bg-blue-700' }} text-white font-extrabold text-xs shadow-xs transition-all">
