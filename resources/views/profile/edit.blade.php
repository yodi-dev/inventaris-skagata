        ...
        <div class="bg-white rounded-3xl border border-gray-200/80 shadow-xs overflow-hidden">
            ...
            <div class="px-5 sm:px-7 pb-6 -mt-10 sm:-mt-12">
                <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-5">
                        ...
                        <div class="mb-1.5">
                            <div class="flex flex-wrap items-center gap-2">
                                <h1 class="text-lg sm:text-2xl font-black text-gray-900 leading-tight"
                                    x-text="profile.nama"></h1>
                                <span
                                    class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                    ...
                    {{-- Role Badge --}}
                    <span
                        class="self-start sm:self-auto inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold tracking-wide border shadow-2xs mt-1 sm:mt-0"
                        :class="{
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-4 border-t border-gray-100">
                    <template x-for="(stat, i) in profile.stats" :key="i">
                        <div class="bg-slate-50 border border-gray-200/70 rounded-2xl p-3 text-center sm:text-left">
                            <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wide truncate"
                                x-text="stat.label"></p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">Nama
                            Lengkap</label>
                        <input type="text" x-model="profile.nama" required
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-bold text-gray-600 uppercase tracking-wide"
                                x-text="profile.identifierLabel"></label>
                        ...
                        <input type="text" x-model="profile.identifier" readonly
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">Alamat
                            Email</label>
                        <input type="email" x-model="profile.email" placeholder="email@smkn3yk.sch.id"
                            class="w-full text-xs sm:text-sm border border-gray-300 rounded-xl py-3 px-3.5 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 font-medium transition-all outline-none">
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">No. WhatsApp /
                            HP</label>
                        <input type="text" x-model="profile.phone" placeholder="08xxxxxxxxxx"
                            class="w-full text-xs sm:text-sm border border-gray-300 rounded-xl py-3 px-3.5 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 font-medium transition-all outline-none">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">
                            <span
                                x-text="activeRole === 'peminjam' ? 'Bengkel / Jurusan' : (activeRole === 'toolman' ? 'Bengkel Tanggung Jawab' : 'Unit Kerja')"></span>
                        </label>
                        <input type="text" x-model="profile.bengkel" readonly
                ...
                <div class="max-w-xl space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">Kata Sandi Saat
                            Ini</label>
                        <div class="relative">
                            <input :type="showOld ? 'text' : 'password'" x-model="pwd.old" required
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">Kata Sandi
                            Baru</label>
                        <div class="relative">
                            <input :type="showNew ? 'text' : 'password'" x-model="pwd.new" required minlength="8"
                        ...
                        <div class="mt-2 space-y-1">
                            ...
                            <p class="text-[10px] text-gray-400">
                                ...
                                <span x-show="pwd.new.length > 0 && pwd.new.length < 6" class="text-rose-500">Terlalu
                                    pendek</span>
                                <span x-show="pwd.new.length >= 6 && pwd.new.length < 8" class="text-amber-600">Hampir
                                    cukup…</span>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">Konfirmasi Kata
                            Sandi Baru</label>
                        <input type="password" x-model="pwd.confirm" required
                            placeholder="Ketik ulang kata sandi baru..."
                            class="w-full text-xs sm:text-sm border border-gray-300 rounded-xl py-3 px-3.5 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 font-medium transition-all outline-none"
                            :class="pwd.confirm && pwd.new !== pwd.confirm ?
                                'border-rose-400 focus:ring-rose-400 focus:border-rose-400' : ''">
                        <p x-show="pwd.confirm && pwd.new !== pwd.confirm" class="text-[10px] text-rose-500 mt-1">Kata
                            sandi tidak cocok.</p>
                    </div>
        ...
        function profileApp(initialRole) {
            const DATA = {
                ...
                toolman: {
                    ...
                    stats: [{
                            label: 'Barang Dikelola',
                            value: '124 Jenis'
                        },
                        {
                            label: 'Antrean Pinjam',
                            value: '8 Tiket'
                        },
                        {
                            label: 'Usulan RAB',
            ...
            return {
                ...
                activeTab: 'profile',
                profile: structuredClone ? structuredClone(DATA[initialRole] ?? DATA.peminjam) : JSON.parse(JSON.stringify(
                    DATA[initialRole] ?? DATA.peminjam)),
