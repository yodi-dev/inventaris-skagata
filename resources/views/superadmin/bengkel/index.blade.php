...
@section('title', 'Master Data Bengkel')
@section('header_title', 'Master Data Bengkel')

@section('content')
    <script>
        function bengkelData() {
            return {
                ...
                deleteKode: '',
                deleteAction: '',
                openDelete(id, nama, kode) {
                    this.deleteId = id;
                    this.deleteNama = nama;
                    this.deleteKode = kode;
                    this.deleteAction = '{{ url('superadmin/bengkel') }}/' + id;
                    this.showDeleteModal = true;
                }
            };
        }
        window.bengkelData = bengkelData;
        if (window.Alpine) {
            window.Alpine.data('bengkelData', bengkelData);
        } else {
    ...
    <div x-data="bengkelData()" class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Master Data Bengkel & Jurusan</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola unit bengkel kejuruan tempat penyimpanan aset, BHP, dan sirkulasi peminjaman.</p>
            </div>

            <!-- Action Button Tambah -->
            <a href="{{ route('superadmin.bengkel.create') }}"
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
            <form action="{{ route('superadmin.bengkel.index') }}" method="GET"
                ...
                <div class="relative w-full sm:w-96">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama, kode bengkel, atau deskripsi..."
            ...
            <div class="text-sm text-gray-500 whitespace-nowrap">
                Total: <span class="font-bold text-gray-800">{{ $bengkels->total() }}</span> Bengkel Terdaftar
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 text-gray-600 text-sm border-b border-gray-200">
                            ...
                            <th class="py-3 px-4 font-semibold">Staf Toolman</th>
                        ...
                        @forelse ($bengkels as $index => $bengkel)
                            <tr class="hover:bg-slate-50 transition-colors group">
                                <td class="py-4 px-4 text-center font-medium text-gray-500">
                                    {{ ($bengkels->currentPage() - 1) * $bengkels->perPage() + $index + 1 }}
                                </td>
                                <td class="py-4 px-4">
                                    <div class="flex items-start gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-green-50 text-green-700 border border-green-200 flex items-center justify-center shrink-0 font-bold">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2 mt-1">
                                                <span class="bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded font-mono font-semibold border border-gray-200">
                                                    {{ $bengkel->kode }}
                                                </span>
                                                @if ($bengkel->deskripsi)
                                                    <span class="text-xs text-gray-400 truncate max-w-xs" title="{{ $bengkel->deskripsi }}">
                                                        • {{ $bengkel->deskripsi }}
                                                    </span>
                                                @endif
                                            </div>
                                ...
                                <td class="py-4 px-4">
                                    @if ($bengkel->users->isNotEmpty())
                                        <div class="space-y-1">
                                            @foreach ($bengkel->users->take(2) as $toolman)
                                                <div class="flex items-center gap-2 text-xs">
                                                    <div class="w-2 h-2 rounded-full bg-emerald-500 shrink-0"></div>
                                                    <span class="font-medium text-gray-800">{{ $toolman->name }}</span>
                                                    @if ($toolman->nomor_identitas)
                                                        <span class="text-[11px] text-gray-400 font-mono">({{ $toolman->nomor_identitas }})</span>
                                                    @endif
                                                </div>
                                            @endforeach
                                            @if ($bengkel->users->count() > 2)
                                                <div class="text-[11px] text-gray-400 pl-4">
                                                    +{{ $bengkel->users->count() - 2 }} staf lainnya
                                                </div>
                                            @endif
                                        </div>
                                    @else
                                        <div class="text-xs text-gray-400 italic flex items-center gap-1.5">
                                ...
                                <td class="py-4 px-4 text-center">
                                    <span class="inline-flex items-center justify-center bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-bold ring-1 ring-inset ring-blue-600/20">
                                        {{ number_format($bengkel->inventaris_count, 0, ',', '.') }} Item
                                    </span>
                                </td>
                                <td class="py-4 px-4 text-center">
                                    <span class="inline-flex items-center justify-center bg-amber-50 text-amber-700 px-3 py-1 rounded-full text-xs font-bold ring-1 ring-inset ring-amber-600/20">
                                ...
                                <td class="py-4 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <button type="button"
                                            ...
                                            data-kode="{{ $bengkel->kode }}"
                                            @click="openDelete($el.dataset.id, $el.dataset.nama, $el.dataset.kode)"
                                            class="p-2 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 border border-red-200/60 rounded-lg transition-all inline-flex items-center justify-center shadow-xs cursor-pointer"
                                            title="Hapus Bengkel">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <tr>
                                <td colspan="6" class="py-8 px-4 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-base font-medium text-gray-900">Belum ada data Bengkel ditemukan</p>
                                        <p class="text-sm mt-1">Coba sesuaikan kata kunci pencarian atau tambahkan bengkel baru.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-bengkel-delete-title"
                        ...
                        <div class="p-6 sm:p-7">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 pt-0.5">
                                    <h3 class="text-lg font-bold text-gray-900" id="modal-bengkel-delete-title">Hapus Data Bengkel?</h3>
                                    <p class="text-sm text-gray-500 mt-1.5 leading-relaxed">
                                        Apakah Anda yakin ingin menghapus data bengkel berikut secara permanen?
                                    </p>

                                    <!-- Card Item Preview -->
                                    <div class="mt-3.5 p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                                        ...
                                        <div class="inline-flex items-center gap-1.5 mt-1 px-2.5 py-0.5 rounded bg-white border border-gray-200 text-xs font-mono font-medium text-gray-700 shadow-2xs">
                                            <span>Kode:</span>
                                            <span class="text-red-600 font-bold" x-text="deleteKode"></span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-3.5 flex items-center gap-2 p-2.5 bg-red-50/70 border border-red-200/80 rounded-lg text-xs text-red-700">
                                        ...
                                        <span>Penghapusan akan ditolak sistem jika bengkel masih memiliki barang inventaris aktif.</span>
                                    </div>
                                </div>
                            </div>
                        </div>
