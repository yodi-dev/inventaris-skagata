    ...
    <div x-data="{
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Tinjau usulan Rencana Anggaran Biaya (RAB) dari seluruh bengkel dan
                    berikan keputusan disposisi.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div class="p-4 bg-white border border-gray-200 rounded-2xl flex items-center shadow-xs min-w-0">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="text-xl font-bold text-gray-900">{{ number_format($totalRAB, 0, ',', '.') }} <span
                                class="text-xs font-normal text-gray-500">Usulan</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 bg-white border border-amber-200 rounded-2xl flex items-center shadow-xs min-w-0">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    <div
                        class="w-11 h-11 rounded-xl bg-amber-50 text-amber-700 border border-amber-200 flex items-center justify-center font-bold shrink-0 relative">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @if ($totalPending > 0)
                            <span
                                class="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-500 rounded-full animate-ping"></span>
                        @endif
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="text-[11px] font-semibold text-amber-600 uppercase tracking-wider">Menunggu Waka</div>
                        <div class="text-xl font-bold text-amber-700">{{ number_format($totalPending, 0, ',', '.') }} <span
                                class="text-xs font-normal text-amber-600">Pending</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 bg-white border border-orange-200 rounded-2xl flex items-center shadow-xs min-w-0">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    <div
                        class="w-11 h-11 rounded-xl bg-orange-50 text-orange-700 border border-orange-200 flex items-center justify-center font-bold shrink-0">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="text-xl font-bold text-orange-700">{{ number_format($totalRevisi, 0, ',', '.') }} <span
                                class="text-xs font-normal text-orange-600">Berkas</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 bg-white border border-emerald-200 rounded-2xl flex items-center shadow-xs min-w-0">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="text-xl font-bold text-emerald-700">{{ number_format($totalApproved, 0, ',', '.') }}
                            <span class="text-xs font-normal text-emerald-600">Acc</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-4 items-end">
                    ...
                    <div class="sm:col-span-1 lg:col-span-3">
                        ...
                        <select id="bengkel_id" name="bengkel_id" onchange="this.form.submit()"
                            ...
                            @foreach ($bengkels as $b)
                                <option value="{{ $b->id }}"
                                    {{ (string) $filterBengkel === (string) $b->id ? 'selected' : '' }}>
                                    {{ $b->nama }} ({{ $b->kode }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-1 lg:col-span-3">
                        ...
                        <select id="status" name="status" onchange="this.form.submit()"
                            ...
                            <option value="pending" {{ $filterStatus === 'pending' ? 'selected' : '' }}>Menunggu Disposisi
                                (Pending)</option>
                            <option value="revisi" {{ $filterStatus === 'revisi' ? 'selected' : '' }}>Perlu Revisi
                                (Revisi)</option>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-xs text-gray-500 line-clamp-1 mt-0.5">
                                        {{ $namaBarangList ?: 'Belum ada rincian item' }}</p>
                                </td>

                                {{-- Kuantitas & Estimasi Biaya --}}
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <p class="font-bold text-gray-900">{{ $totalItemsCount }} Item ({{ $totalQty }}
                                        unit)</p>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-2 text-gray-400">
                                        ...
                                        <p class="text-sm font-semibold text-gray-600">Tidak ada pengajuan RAB yang cocok.
                                        </p>
                                        <p class="text-xs text-gray-400 max-w-sm">
                                            Tidak ditemukan berkas RAB dengan filter yang dipilih. Coba atur ulang status
                    ...
                    <div class="p-6 border-b border-gray-100 pr-12">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1 min-w-0 pt-0.5">
                                ...
                                <p class="text-xs text-gray-500 mt-1">Pengadaan ini akan disetujui untuk realisasi
                                    kebutuhan bengkel.</p>
                            </div>
                        </div>
                    ...
                    <div class="p-6 border-b border-gray-100 pr-12">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1 min-w-0 pt-0.5">
                                ...
                                <p class="text-xs text-gray-500 mt-1">Minta toolman bengkel untuk memperbaiki jumlah
                                    kuantitas, spesifikasi, atau estimasi harga.</p>
                            </div>
                        </div>
                ...
                <div x-show="showRejectModal"
                    ...
                    <form :action="activeItem.reviewUrl" method="POST" class="p-6 space-y-4">
                        ...
                        <div
                            ...
                            <span>Status usulan akan berubah menjadi ditolak dan catatan ini akan terbaca oleh Toolman
                                pengusul.</span>
                        </div>
