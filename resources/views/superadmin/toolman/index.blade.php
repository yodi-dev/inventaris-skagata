    ...
    <div x-data="{
        ...
        <div
            ...
            <form action="{{ route('superadmin.toolman.index') }}" method="GET"
                ...
                <select name="bengkel" onchange="this.form.submit()"
                    class="w-full sm:w-56 rounded-lg border-gray-300 focus:border-green-600 focus:ring-green-600 text-sm py-2 px-3 border shadow-sm outline-none transition-all bg-white">
                    ...
                    <tbody class="text-sm text-gray-700 divide-y divide-gray-100">
                        ...
                        @forelse ($toolmans as $index => $toolman)
                            @php
                                ...
                                $bengkelKode = $toolman->bengkel->kode ?? '-';
                            @endphp
                            <tr class="hover:bg-slate-50 transition-colors group">
                                ...
                                <td class="py-4 px-4">
                                    <div class="flex items-center gap-3">
                                        <!-- Avatar Initial -->
                                        <div
                                            class="h-10 w-10 rounded-full {{ $toolman->status === 'aktif' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200' }} flex items-center justify-center font-bold border">
                                            {{ $initials }}
                                        </div>
                                        <div>
                                            <div class="font-bold text-gray-900">{{ $toolman->name }}</div>
                                            <div class="text-xs text-gray-500 flex items-center gap-2 mt-0.5">
                                                <span>{{ $toolman->nomor_identitas ? 'NIP. ' . $toolman->nomor_identitas : 'NIP. -' }}</span>
                                                <span class="text-gray-300">•</span>
                                                <span class="text-gray-500">{{ $toolman->email }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-4 px-4">
                                    <span
                                        ...
                                        {{ $toolman->bengkel ? $toolman->bengkel->nama : 'Belum Ditentukan' }}
                                    </span>
                                </td>
                                <td class="py-4 px-4 text-center">
                                    ...
                                    @elseif($toolman->status === 'menunggu_acc')
                                        <span
                                    ...
                                    @else
                                        <span
                                            ...
                                            {{ ucfirst($toolman->status ?? 'Nonaktif') }}
                                        </span>
                                    @endif
                                </td>
                                <td class="py-4 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <button type="button"
                                            @click="openReset('{{ addslashes($toolman->name) }}', '{{ addslashes($bengkelNama) }}', '{{ addslashes($toolman->email) }}')"
                                        ...
                                        <button type="button"
                                            @click="openDelete('{{ addslashes($toolman->name) }}', '{{ addslashes($bengkelNama) }}', '{{ addslashes($toolman->email) }}')"
                            ...
                            <tr>
                                <td colspan="5" class="py-8 px-4 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm mt-1">Coba sesuaikan kata kunci pencarian atau filter bengkel.
                                        </p>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <div class="flex-1 pt-0.5">
                                ...
                                <div class="mt-3.5 p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1.5">
                                    ...
                                    <div class="flex items-center gap-2 pt-1 text-xs text-gray-600">
                                        <span class="px-2 py-0.5 rounded bg-white border border-gray-200 font-medium"
                                            x-text="deleteBengkel"></span>
        ...
        <div x-show="showResetModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto"
            ...
            <div x-show="showResetModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity" @click="showResetModal = false">
            </div>
                ...
                <div x-show="showResetModal" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="p-6 sm:p-7 border-b border-gray-100">
                        <div class="flex items-start gap-4 pr-10">
                            <!-- Icon Kunci Amber -->
                            <div
                                class="w-12 h-12 rounded-2xl bg-amber-100 text-amber-700 flex items-center justify-center shrink-0 border border-amber-200/80 ring-4 ring-amber-50/80 shadow-xs">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z">
                                    </path>
                        ...
                        <div
                            class="mt-4 p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl flex items-center justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                <div
                                    class="w-9 h-9 rounded-lg bg-green-100 text-green-700 font-bold text-xs flex items-center justify-center shrink-0 border border-green-200">
                                    <span x-text="resetNama ? resetNama.substring(0,2).toUpperCase() : 'TM'"></span>
                                </div>
                                <div class="min-w-0">
                                    <div class="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Akun
                                        Sasaran:</div>
                            ...
                            <span
                                class="px-2.5 py-1 rounded-md bg-white border border-gray-200 text-xs font-semibold text-slate-700 shadow-2xs shrink-0"
                    ...
                    <form @submit.prevent="confirmReset()" class="p-6 sm:p-7 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input :type="showNewPassword ? 'text' : 'password'" id="modal_new_password"
                                    x-model="newPassword" required minlength="8" placeholder="Minimal 8 karakter"
                                ...
                                <button type="button" @click="showNewPassword = !showNewPassword"
                                    ...
                                    <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                        x-show="showNewPassword" style="display: none;">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input :type="showNewPassword ? 'text' : 'password'" id="modal_confirm_password"
                                    x-model="newPasswordConfirmation" required minlength="8"
                                    placeholder="Ketik ulang password baru"
                                    class="w-full pl-3.5 pr-11 py-2.5 rounded-lg border border-gray-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 text-sm shadow-sm font-mono tracking-wide">
                                <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none">
                                    <span
                                        x-show="newPassword && newPasswordConfirmation && newPassword === newPasswordConfirmation"
                        ...
                        <div
                            class="p-3 bg-amber-50/80 border border-amber-200/80 rounded-xl flex items-start gap-2.5 text-xs text-amber-900 leading-relaxed">
                            ...
                            <span>
                                Password baru akan langsung aktif setelah disimpan. Pastikan untuk mencatat dan memberikan
                                password ini kepada staf toolman terkait.
                            </span>
                        </div>
